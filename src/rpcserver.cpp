...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Flexinodes server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Flexinodes server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* FLX features */
        {"flexinodes", "masternode", &masternode, true, true, false},
        {"flexinodes", "masternodelist", &masternodelist, true, true, false},
        {"flexinodes", "mnbudget", &mnbudget, true, true, false},
        {"flexinodes", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"flexinodes", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"flexinodes", "mnsync", &mnsync, true, true, false},
        {"flexinodes", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"flexinodes", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use flexinodesd, or the -server option to flexinodes-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
                                               "It is recommended you use the following random password:\n"
                                               "rpcuser=flexinodesrpc\n"
                                               "rpcpassword=%s\n"
                                               "(you do not need to remember this password)\n"
                                               "The username and password MUST NOT be the same.\n"
                                               "If the file does not exist, create it with owner-readable-only file permissions.\n"
                                               "It is also recommended to set alertnotify so you are notified of problems;\n"
                                               "for example: alertnotify=echo %%s | mail -s \"Flexinodes Alert\" admin@foo.com\n"),
...
{
    return "> flexinodes-cli " + methodname + " " + args + "\n";
}
